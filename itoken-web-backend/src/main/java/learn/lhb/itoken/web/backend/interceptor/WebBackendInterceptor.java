package learn.lhb.itoken.web.backend.interceptor;
...
import learn.lhb.itoken.web.backend.service.RedisService;
...
//                response.sendRedirect(String.format("%s/login?url=%s",HOSTS_SSO, HttpServletUtils.getFullPath(request)));
                response.sendRedirect("http://localhost:8503/login?url=http://localhost:8603"+request.getServletPath());
            } catch (IOException e) {
                e.printStackTrace();
            }
            return false;
